...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function logic(tableData) {
    const soilWater = {
        Silt: randomNumber(22.5, 27.5),
        Sand: randomNumber(12, 16),
        Clay: randomNumber(30, 50),
      },
      soilDensity = { Silt: 1.46, Sand: 1.43, Clay: 1.39 };
    tableData.forEach(function (row, index) {
      const waterContent = soilWater[row["Soil Type"]],
        ans = soilDensity[row["Soil Type"]];
      row["Wet Soil Mass(g)"] = (ans * (1 + waterContent / 100) * 800).toFixed(
        2,
      );
  ...
  function cutting() {
    ...
    if (
      objs["rammer"].pos[1] + objs["rammer"].height ===
      objs["dolly"].pos[1]
    ) {
      if (
        extras["soilPart"].pos[1] + extras["soilPart"].height <=
        objs["cutter"].pos[1] + objs["cutter"].height - translate[1]
      ) {
    ...
    if (translate[0] === 0 && translate[1] === 0) {
      if (step === 2) {
        document.getElementById("output1").innerHTML =
          "Mass of cutter = " + String(10) + " g";
      } else if (step === enabled.length - 2) {
        document.getElementById("output2").innerHTML =
          "Volume of soil = 800 cm" + "3".sup();
        logic(tableData);
  ...
  class soilPart {
    ...
    draw(ctx) {
      ctx.drawImage(
        extras["soilPart"].img,
        extras["soilPart"].pos[0],
        extras["soilPart"].pos[1],
        extras["soilPart"].width,
        extras["soilPart"].height,
      );
    }
  ...
  function init() {
    ...
    enabled = [
      ...
      ["cutter", "soil"],
      ["cutter", "soil", "dolly"],
      ["cutter", "soil", "dolly", "rammer"],
      ["cutter", "soil", "dolly", "rammer", "soilPart"],
      ["weight", "cutter", "soilPart"],
      [],
    ];
    step = 0;
    translate = [0, 0];
  ...
  function check(event, translate, step, flag = true) {
    ...
    const canvasPos = [
      (canvas.width / canvas.offsetWidth) * (event.pageX - canvas.offsetLeft),
      (canvas.height / canvas.offsetHeight) * (event.pageY - canvas.offsetTop),
    ];
    const errMargin = 10;
    ...
    keys.forEach(function (val, ind, arr) {
      if (
        canvasPos[0] >= objs[val].pos[0] - errMargin &&
        canvasPos[0] <= objs[val].pos[0] + objs[val].width + errMargin &&
        canvasPos[1] >= objs[val].pos[1] - errMargin &&
        canvasPos[1] <= objs[val].pos[1] + objs[val].height + errMargin
      ) {
  ...
  const msgs = [
    ...
    "Click on the cutter with soil to weigh it. Finally, determine the water content of the soil sample. Use the following <a href='https://smfe-iiith.vlabs.ac.in/exp/water-content/'>link</a> to learn more about water content determination.",
  ...
  const objNames = Object.keys(objs),
    extrasNames = Object.keys(extras);
  ...
  canvas.addEventListener("mousemove", function (event) {
    check(event, translate, step, false);
  });
